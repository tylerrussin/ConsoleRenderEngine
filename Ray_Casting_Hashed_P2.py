...
class Game():
    ...
    def on_user_update(self):
        ...
        try:
            char_list = [char for char in screen_string]
            index = 0
            for val in char_list:
                x, y = math.modf(index / self.screen_width)
                y = int(y)
                x = int(x * self.screen_width)
                if val == 'R':
                    self.view.addstr(y, x, ' ', curses.color_pair(2))
                elif val == 'W':
                    self.view.addstr(y, x, ' ', curses.color_pair(3))
                elif val == 'G':
            ...
            screen_string = self.hashed_map.get("({}, {}, {})".format(round(self.player_a, 2), round(self.player_y, 1), round(self.player_x, 1)))
            self.screen = [char for char in screen_string]
            index = 0
            for val in self.screen:
                x, y = math.modf(index / self.screen_width)
                y = int(y)
                x = int(x * self.screen_width)
                if val == 'R':
